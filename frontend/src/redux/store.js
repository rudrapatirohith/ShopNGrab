...
export const store= configureStore({   //Exports the Redux store named store configured with configureStore
    ...
    middleware: (getDefaultMiddleware) => getDefaultMiddleware().concat([productApi.middleware,authApi.middleware,userApi.middleware]),  // Configures the middleware by concatenating the default middleware with productApi.middleware.
}) 